...
def patch_backend_route() -> None:
    if BACKEND_ROUTE.exists():
        original = BACKEND_ROUTE.read_text(encoding="utf-8")
        if original != ACTIVITY_ROUTE:
            BACKEND_ROUTE.with_suffix(".py.activity_v1.bak").write_text(original, encoding="utf-8")
    BACKEND_ROUTE.write_text(ACTIVITY_ROUTE, encoding="utf-8")
    print("Wrote backend/app/routes/activity.py")
...
def patch_backend_main() -> None:
    ...
    if "activity" not in text:
        route_import_lines = [line for line in text.splitlines() if line.startswith("from app.routes import ")]
        if route_import_lines:
            line = route_import_lines[0]
            text = text.replace(line, line + ", activity", 1)
        else:
            # Insert near the top after normal imports.
            lines = text.splitlines()
            insert_at = 0
            for i, line in enumerate(lines):
                if line.startswith("import ") or line.startswith("from "):
                    insert_at = i + 1
            lines.insert(insert_at, "from app.routes import activity")
            text = "\n".join(lines) + ("\n" if original.endswith("\n") else "")

    if "app.include_router(activity.router)" not in text:
        include_lines = [line for line in text.splitlines() if "app.include_router(" in line]
        if include_lines:
            last = include_lines[-1]
            text = text.replace(last, last + "\napp.include_router(activity.router)", 1)
        else:
            text += "\napp.include_router(activity.router)\n"

    if text != original:
        BACKEND_MAIN.with_suffix(".py.activity_v1.bak").write_text(original, encoding="utf-8")
...
def patch_frontend() -> None:
    ...
    text = text[:start] + ACTIVITY_DASHBOARD + text[end:]

    if text != original:
        FRONTEND_MAIN.with_suffix(".tsx.activity_v1.bak").write_text(original, encoding="utf-8")
    ...
    if "Activity page v1" not in css:
        FRONTEND_CSS.with_suffix(".css.activity_v1.bak").write_text(css, encoding="utf-8")
...
def main() -> None:
    ...
    print("")
    print("Done. Restart backend/frontend and open the Activity tab.")
