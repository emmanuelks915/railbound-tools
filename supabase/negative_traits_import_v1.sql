-- Negative Traits Import v6 FULL REBUILD
-- Source: Railbound Trait Guide, Negative Traits section.
-- This version does NOT use ON CONFLICT.
-- It deletes only the 24 negative trait keys for this guild, then inserts them fresh.
-- Includes required guild_id and slug.
...
alter table public.traits add column if not exists requirements jsonb default '{}'::jsonb;
...
(
  ...
  '{"mechanical_effects": ["Skills cost 5% more XP."], "restrictions": [], "incompatible_traits": [], "notes": "Cleaned from source wording.", "source": "Railbound Trait Guide - Negative Traits", "requires_staff_approval": true, "trait_type": "negative", "tier": 1, "refund_points": 1}'::jsonb,
  '{}'::jsonb,
  true,
  true
),
...
(
  ...
  '{"mechanical_effects": ["-1 to all social rolls."], "restrictions": [], "incompatible_traits": [], "notes": "Cleaned typo from source.", "source": "Railbound Trait Guide - Negative Traits", "requires_staff_approval": true, "trait_type": "negative", "tier": 1, "refund_points": 1}'::jsonb,
  '{}'::jsonb,
  true,
  true
),
...
(
  ...
  '{"mechanical_effects": ["-2 Luck on skill rolls when near drops that are 10+ feet or greater."], "restrictions": ["Cannot enter or pilot floating, suspended, or raising platforms or vehicles alone."], "incompatible_traits": [], "notes": "Source appears to omit the minus sign before 2 Luck; imported as -2 Luck to match other severe phobia traits.", "source": "Railbound Trait Guide - Negative Traits", "requires_staff_approval": true, "trait_type": "negative", "tier": 3, "refund_points": 3}'::jsonb,
  '{}'::jsonb,
  true,
  true
),
