...
def get_companion(character_id: UUID, actor_discord_id:int|None=Depends(actor_from_header)):
    ...
    return {"eligible":elig,"character":{"character_id":cid,"name":c.get("name")},"loyal_companion_trait":next((t for t in traits if _eligible([t])),None),"beast":beast,"oc_stats":stats,"computed_stats":_computed(beast,stats),"type_rules":RULES.get(typ,RULES["utility"]),"allowed_types":["combat","mount","utility"],"wallet":(lambda r:r[0] if r else {"available_xp":0})(_safe(sb.table("oc_xp_wallets").select("available_xp,total_spent_xp").eq("character_id",cid).limit(1))),"beast_xp_spent":sum(int(x.get("cost") or 0) for x in _safe(sb.table("skill_purchase_requests").select("cost").eq("character_id",cid).in_("source_label",["Beast Skill","Beast Stat"]).eq("status","approved").limit(500)))}
...
def request_beast_skill(character_id: UUID, payload: dict=Body(default={}), actor_discord_id: int|None=Depends(actor_from_header)):
    ...
    rows=_safe(sb.table("oc_xp_wallets").select("available_xp,total_spent_xp").eq("character_id",cid).limit(1)); wallet=rows[0] if rows else {"available_xp":0}
